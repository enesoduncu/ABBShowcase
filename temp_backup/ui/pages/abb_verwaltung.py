...
from models.abb import SchulabschlussEnum, StudiumStatusEnum
    ...
    with st.form("abb_form"):
        ...
        with col2:
            schulabschluss = st.selectbox(
                "Schulabschluss", 
                ["Bitte wählen..."] + [e.value for e in SchulabschlussEnum],
                index=0
            )
            vorbildung = st.text_input("Vorbildung", max_chars=200, help="Vorangegangene Ausbildung")
            studium_status = st.selectbox(
                "Studium-Status", 
                ["Bitte wählen..."] + [e.value for e in StudiumStatusEnum],
                index=0
            )
            studium_fach = st.text_input("Studium-Fach", max_chars=200, help="Welches Studium abgebrochen/abgeschlossen")
        ...
        with col2:
            notizen = st.text_area("Notizen für Koordinator*innen", max_chars=1000, help="z.B. 'Kann gut vor großen Gruppen sprechen', 'Spezialist für IT-Berufe'")
            ...
            if validate_abb_form(vorname, nachname, bereich):
                try:
                    abb_data = ABBCreate(
                        ...
                        schulabschluss=schulabschluss if schulabschluss != "Bitte wählen..." else None,
                        vorbildung=vorbildung,
                        studium_status=studium_status if studium_status != "Bitte wählen..." else None,
                        studium_fach=studium_fach,
                        beruf=beruf,
                        zq=zq,
                        bereich=bereich,
    ...
    try:
        ...
        for abb in abbs:
            data.append({
                ...
                'Studium-Status': abb.get('studium_status', ''),
                'Studium-Fach': abb.get('studium_fach', ''),
                'Beruf': abb.get('beruf', ''),
                'ZQ': abb.get('zq', ''),
